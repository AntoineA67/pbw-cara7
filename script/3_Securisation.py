...
def scoring():
	...
	def score_one_car(car_data):
		...
		# Scoring criteria
		if battery_health > 90 and total_kilometers < 30000 and not payment_default and late_payment_months < 2:
			return 'A'
		elif 80 <= battery_health <= 90 and total_kilometers < 40000 and late_payment_months < 4:
			return 'B'
		else:
			return 'C'

	# Example usage
	car_scores = {filename: score_one_car(car_info) for filename, car_info in cars_data.items()}
	print(car_scores)
